(function ($, Drupal) {
    Drupal.behaviors.myCustomBehavior = {
      attach: function (context, settings) {
        // Your existing code here, modified to ensure compatibility
        // For example:
       ...
       /* --------------------------------------------
        ...
        // If you have code that needs to run on window.load,
        // consider using once() to ensure it's not attached multiple times
        $(window).on('load', function() {
            $(window).trigger("scroll");
            $(window).trigger("resize");
        });
...
/* ---------------------------------------------
 ...
 --------------------------------------------- */
 (function ($, Drupal) {
    Drupal.behaviors.initButtonAnimation = {
      attach: function (context, settings) { 
...
(function ($, Drupal) {
            Drupal.behaviors.initParallaxMousemove = {
              attach: function (context, settings) {
                const isReduced = window.matchMedia("(prefers-reduced-motion: reduce)") === true || window.matchMedia("(prefers-reduced-motion: reduce)").matches === true;
 ...
 /* ---------------------------------------------
        ...
        // Projects filtering
        (function ($, Drupal) {
            Drupal.behaviors.projectFiltering = {
              attach: function (context, settings) {
                var fselector = 0;
                var work_grid = $("#work-grid, #isotope", context);
          
                var init_work_filter = function () {
                  "use strict";
                  var isotope_mode;
                  ...
                  $(".filter", context).click(function () {
                    $(".filter", context).removeClass("active").attr("aria-pressed", "false");
                  ...
                  // Initial filter selection if hash is present in the URL.
                  ...
                  // Update Isotope layout on image load (for lazy-loaded images).
                  $(".img-lazy-work", context).on("load", function () {
                    masonry_update();
                  });
                  ...
                  // Ensure Isotope layout updates on window resize.
                  $(window).resize(function () {
                    setTimeout(() => {
                      masonry_update();
                    }, 700);
                ...
                // Execute the filter initialization function.
                init_work_filter();
              }
            };
          })(jQuery, Drupal);
 ...
 /* ---------------------------------------------
        ...
        --------------------------------------------- */
(function ($, Drupal) {
    Drupal.behaviors.initWowAnimation = {
      attach: function (context, settings) {
        setTimeout(() => {
          // Wow init
          if ($("body").hasClass("appear-animate")) {
            $(".wow", context).addClass("no-animate");
          }
          var wow = new WOW({
            boxClass: 'wow',
          ...
          if ($("body").hasClass("appear-animate")) {
            ...
            $(".wow", context).css("opacity", "1");
          }
  
          // Wow for portfolio init
          if ($("body").hasClass("appear-animate")) {
            $(".wow-p", context).addClass("no-animate");
          }
          var wow_p = new WOW({
            boxClass: 'wow-p',
            animateClass: 'animated',
          ...
          if ($("body").hasClass("appear-animate")) {
            ...
            $(".wow-p", context).css("opacity", "1");
          }
  
          // Wow for menu bar init
          if ($("body").hasClass("appear-animate") && $(window).width() >= 1024 && $("html").hasClass("no-mobile")) {
            $(".wow-menubar", context).addClass("no-animate fadeInDown animated");
            setInterval(function () { $(".wow-menubar").removeClass("no-animate"); }, 1500);
          } else {
            $(".wow-menubar", context).css("opacity", "1");
          }
  
        }, 100);
  
        // Splitting JS accessibility fix
        $("[data-splitting='chars']", context).each(function () {
          var splitting_chars = $(this);
          var splitting_chars_cont = splitting_chars.text(); // Use text() instead of html() to ensure we're not duplicating any HTML entities
...
(function ($, Drupal) {
    Drupal.behaviors.iconAccessibilityEnhancement = {
      attach: function (context, settings) {
            /* ---------------------------------------------
            Masonry
            --------------------------------------------- */
...
/* ---------------------------------------------
 ...
 (function ($, Drupal) {
    Drupal.behaviors.iconAccessibilityEnhancement = {
      attach: function (context, settings) {
        var getIcons = context.querySelectorAll('i[class^="mi-"], i[class*=" mi-"], i[class^="icon-"], i[class*=" icon-"], i[class^="fa-"], i[class*=" fa-"]');
...
(function ($, Drupal) {
  ...
  (function ($, Drupal) {
    Drupal.behaviors.customThemeBehavior = {
      attach: function (context, settings) {
        $('.some-selector', context).each(function () {
  ...
  (function ($, Drupal) {
    Drupal.behaviors.customTeamBehavior = {
      attach: function (context, settings) {
        // Initialize team functionality only once per context
         // Team carousel
        ...
        $(".slider-bold").each(function(){
          ...
          // Adjust team item description height dynamically
          $(".team-item-descr", context).each(function() {
            var descrHeight = $(this).height();
            $(this).closest('.team-carousel').css("--team-item-descr-height", descrHeight + "px");
          });
  
          // Handle click for mobile
          $(".team-item", context).click(function() {
          ...
          $(".team-social-links > a", context).on("focus blur", function() {
            if (!($("html").hasClass("mobile"))) {
              $(this).parents('.team-item').toggleClass("js-active");
            }
          });
          function owl_keynav(el){            
            el.attr({
  ...
  (function ($, Drupal) {
    Drupal.behaviors.classicMenu = {
      attach: function (context, settings) {
        // Ensure code runs only once per element
        var mobile_nav = $(".mobile-nav");
        var desktop_nav = $(".desktop-nav");
        
        mobile_nav.attr("aria-expanded", "false");
        // Run on attach and on window resize
        ...
        function init_classic_menu_resize() {
          ...
          $(".mobile-on .desktop-nav > ul", context).css("max-height", $(window).height() - $(".main-nav", context).height() - 20 + "px");
  
          // Mobile menu style toggle
          if ($(window).width() <= 1024) {
            $(".main-nav", context).addClass("mobile-on");
            if (!mobile_nav.hasClass("active")) {
              desktop_nav.css("display", "none");
            }
          } else if ($(window).width() > 1024) {
            $(".main-nav", context).removeClass("mobile-on");
       ...
       // init_classic_menu_resize();
        ...
        // Initialize classic menu functionalities
        init_classic_menu();
        ...
        // You can continue with other functions like init_classic_menu and init_scroll_navigate here, 
        // making sure to adjust them to use the 'context' and '.once' as shown above.
      }
    };
  })(jQuery, Drupal);
