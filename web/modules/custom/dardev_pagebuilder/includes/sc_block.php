...
function sc_blockbulider($attr){
   ...
   $_id = 'blockbulider-' . dardev_builder_makeid();
   $output = '';
   $output .= "<div class=\"block-builder-main\" id=\"{$_id}\"><div class=\"dardev-blockbuilder-content\">";
   	if($name){ 
   		  $results = dardev_builder_load_by_machine($name);
        ...
        if($user->hasPermission('administer dardev_pagebuilder')){
          $edit_url = Url::fromRoute('dardev_builder.admin.edit', array('bid' => $results->id, 'destination' =>  $url))->toString();
          $output .= "<a class=\"link-edit-blockbuider\" href=\"{$edit_url}\"> Config block builder </a>";
        }
        $output .= dardev_builder_frontend($results->params);
        $results = null;
     	}
   $output .= "</div></div>";
   return $output;
