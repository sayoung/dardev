<?php
function dardev_sliderlayer_datatransition(){
    return $datatransition = array(
        ...
        'papercut' => 'Paper Cut',
        'flyin' => 'Fly In',
        'slideoverup' => 'Slide Over Top',
        'slideoverdown' => 'Slide Over Bottom',
        'slideoverright' => 'Slide Over Right',
        'slideoverleft' => 'Slide Over Left',
        'slidingoverlayup'  => 'Slider Overlays To Top',
        'slidingoverlaydown'  => 'Slider Overlays To Bottom',
        'slidingoverlayleft'  => 'Slider Overlays To Left',
        'slidingoverlayright'  => 'Slider Overlays To Right'
        );
    }

function dardev_sliderlayer_dataeasing_slide(){
...
function dardev_sliderlayer_dataeasing(){
    return array(
        'Linear.easeNone' => 'Linear.easeNone',
...
function dardev_sliderlayer_dataendeasing(){
...
function dardev_sliderlayer_incomingclasse(){
...
function dardev_sliderlayer_outgoingclasses(){
...
function dardev_sliderlayer_captionclasses(){
...
function dataFrameLayer($animation_name){

}

function dardevGetArrAnimations(){
    $arrAnimations = array();
    ...
    foreach($arrAnimations as $key => $value){
        ...
        $params = $value['params'];
        $search = array('movex', 'movey', 'movez', 'rotationx', 'rotationy', 'rotationz', 'scalex', 'scaley', 'skewx', 'skewy', 'captionopacity');
        $replace = array('x_start', 'y_start', 'z_start', 'x_rotate_start','y_rotate_start','z_rotate_start', 'scale_x_start','scale_y_start','skew_x_start', 'skew_y_start', 'opacity_start');
        $params = str_replace($search, $replace, $params);

        $search = array('mask_x', 'mask_y', 'mask_speed');
        $replace = array('mask_x_start', 'mask_y_start', 'mask_speed_start');
        $params = str_replace($search, $replace, $params);
       

        $t = json_decode(str_replace("'", '"', $params), true);
        if(!empty($t))
            $arrAnimations[$key]['params'] = $t;
    }
    //print "<pre>";print_r($arrAnimations);
    return $arrAnimations;
}

 function dardevGetArrEndAnimations(){
        $arrAnimations = array();
        ...
        foreach($arrAnimations as $key => $value){
            ...
            $params = $value['params'];
            $search = array('movex', 'movey', 'movez', 'rotationx', 'rotationy', 'rotationz', 'scalex', 'scaley', 'skewx', 'skewy', 'captionopacity');
            $replace = array('x_end', 'y_end', 'z_end', 'x_rotate_end','y_rotate_end','z_rotate_end', 'scale_x_end','scale_y_end','skew_x_end', 'skew_y_end', 'opacity_end');
            $params = str_replace($search, $replace, $params);

            $search = array('mask_x', 'mask_y', 'mask_speed');
            $replace = array('mask_x_end', 'mask_y_end', 'mask_speed_end');
            $params = str_replace($search, $replace, $params);
    ...
    function parseCustomAnimationByArray($animArray, $is = 'start', $frame_val=''){
        $retString = '';
    ...
    function parseCustomMaskByArray($animArray, $is = 'start'){
        $retString = '';
...
function dardev_defined_select($name, $options, $class = null) {
...
function dardev_sliderlayer_field_upload_slider(){
    global $base_url;
    $_id = dardev_sliderlayer_makeid(10);
    ob_start();
    ?> 
    <div class="drv-upload-input field-upload-background-slider" data-id="<?php print $_id; ?>">
        <input data-id="<?php print $_id; ?>" id="drv-upload-<?php print $_id; ?>" type="text" name="background_image_uri" class="imce-url-input slide-option" data-imce-type="link"/>
        <img class="dardev-image-demo" id="drv-upload-<?php print $_id; ?>-demo" src="" />
        <a class="dardev-field-upload-remove btn-delete" data-src="" style="">Remove</a>
...
function dardev_sliderlayer_field_upload_layer(){
    global $base_url;
    $_id = dardev_sliderlayer_makeid(10);
    ob_start();
    ?> 
    <div class="drv-upload-image drv-upload-image-layer" id="drv-upload-<?php print $_id; ?>">
        <form class="upload-image-layer" id="upload-<?php print $_id; ?>" method="post" action="<?php print ($base_url . '/admin/structure/dardev_sliderlayer/upload') ?>" enctype="multipart/form-data">
            <div class="drop">
        ...
        <div class="clearfix"></div>
        <div class="dardev-box-images">
            <div class="dardev-box-images-inner">
                <div class="header">
                    Images Upload
