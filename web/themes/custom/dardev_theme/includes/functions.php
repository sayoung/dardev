<?php
function dardev_theme_render_block($key) {
...
function dardev_theme_makeid($length = 5){
...
function dardev_theme_base_url(){
  global $base_url;
  $theme_path = \Drupal::service('extension.list.module')->getPath('dardev_theme');
  return $base_url . '/' . $theme_path . '/';
}

global $drv_node_index;
function dardev_theme_preprocess_node(&$variables) {
  global $drv_node_index;
  $drv_node_index = $drv_node_index + 1;
  $variables['drv_node_index'] = $drv_node_index;
  ...
  if ($variables['node']->getType() == 'article') {
      ...
      $variables['drv_iframe'] = $iframe;
      $variables['post_format'] = $post_format;
  }
}

function dardev_theme_preprocess_node__portfolio(&$variables){
  $node = $variables['node'];
...
function dardev_theme_preprocess_node__event(&$variables){
  $node = $variables['node'];
  $event_date = array();
...
function dardev_theme_preprocess_breadcrumb(&$variables){
...
function dardev_theme_print_icon($file, $alt){
