var $settings = drupalSettings.dardev_sliderlayer.settings;
console.log($settings);
var $group_settings = drupalSettings.dardev_sliderlayer.group_settings;

var $layers = drupalSettings.dardev_sliderlayer.layers_settings;
...
var delayer = drupalSettings.dardev_sliderlayer.delayer; 

var deslider = drupalSettings.dardev_sliderlayer.deslider;
...
(function ($) {
  ...
  var ui_slider = document.getElementById('g-slider');
    ...
    //$('#dardev_slider_single').width($group_settings.startwidth).height($group_settings.startheight);
    ...
    $('#prview-option-screen').on('change', function(){
      ...
      $('#dardev_slider_single').addClass('layer-preview-screen' + _screen_class);

      if(_screen == ''){
        $('#dardev_slider_single').width($group_settings.gridwidth).height($group_settings.gridheight);

        var __layer = 0;
        $('#dardev_slider_single > .layer').each(function(){
      ...
      if(_screen == '_sm'){
        $('#dardev_slider_single').width(778).height($group_settings.gridheight_sm);
        var __layer = 0;
        $('#dardev_slider_single > .layer').each(function(){
          $('#layer-' + __layer + ' .inner').css({
      ...
      if(_screen == '_xs'){
        $('#dardev_slider_single').width(480).height($group_settings.gridheight_xs);
        var __layer = 0;
        $('#dardev_slider_single > .layer').each(function(){
          $('#layer-' + __layer + ' .inner').css({
    ...
    $('input#g-image-layer').on('onchange', function(){
        $url = drupalSettings.dardev_sliderlayer.base_url + $(this).val();
  ...
  function setSlideSackground(url){
    jQuery('#dardev_slider_single').css({
...
//========================================
  ...
  function load_slider() {
    ...
    if ($settings.background_image_uri != '') {
      $('#dardev_slider_single').css({
        'background-image': 'url(' + drupalSettings.dardev_sliderlayer.base_url  + $settings.background_image_uri + ')'
      })
      $('.field-upload-background-slider .dardev-image-demo').attr('src', drupalSettings.dardev_sliderlayer.base_url  + $settings.background_image_uri);
  ...
  function drv_sl_append_config_layer(l, i, t){
  ...
  function load_layers() {
    $('#dardev_slider_single').find('div').remove();
    $('#dardev_list_layers').find('li').remove();
  ...
  function add_layer() {
    ...
    drv_sl_add_tab_layer(j);
    load_layer(j);
  }

  function drv_sl_add_tab_layer(i) {
     var layertype = $layers[i].type;
     var t = $('<li>').attr('index', i).addClass(layertype);
        
     drv_sl_append_config_layer($layers[i], i, t);

     $('ul#dardev_list_layers').append(t);
     ...
     switch ($layers[i].type) {
        case 'image':
        var url = drupalSettings.dardev_sliderlayer.base_url  + $layers[i].image_uri;
          content = '<img src="' + url + '" />';
          var img = new Image();
          ...
          img.src = drupalSettings.dardev_sliderlayer.base_url  + $layers[i].image_uri;
          break;
        case 'text':
   ...
   if($layers[i].text_align_lg){
    ...
    newdelayer.click(function () {
     ...
     var _screen = $('#prview-option-screen').val();
     $('#dardev_slider_single').append(newdelayer);
     ...
     $('#dardev_list_layers').sortable({
           handle: '.move',
           update: function (event, ui) {
              $('#dardev_list_layers').find('li').each(function (index) {
                    var lindex = $(this).attr('index');
                    $layers[lindex].index = index;
                    $('#layer-'+lindex).css({zIndex:(99-index)});
                    save_layer();
              });
              $layers.sort(DardevCompare);
              save_slider();
              load_slider();
           }
     });
 ...
 function load_type_layer(type){
    ...
    $('ul#dardev_list_layers li.active').removeClass('image text').addClass(type);
    if (type == 'image') {
  ...
  function load_layer(i) {
    ...
    $('ul#dardev_list_layers').find('li').removeClass('active');
    $('ul#dardev_list_layers').find('li[index=' + i + ']').addClass('active');
  ...
  function remove_layer(i) {
    $('#layer-' + i).remove();
    $layers[i]['removed'] = 1;
    $('ul#dardev_list_layers').find('li[index=' + i + ']').remove();
    if (i == $cxt) {
      if($('ul#dardev_list_layers li').length > 0){
        var f = parseInt($('ul#dardev_list_layers').find('li:first').attr('index'));
        load_layer(f);
      }
    }
  }
  ...
  function save_layer_slider() {
    ...
    $.ajax({
      url: drupalSettings.dardev_sliderlayer.save_url,
      type: 'POST',
      data: data,
      dataType: 'json',
   ...
   function load_imce(){
      ...
      $('.imce-url-button').click(function(e){
        ...
        var inputID = 'drv-upload-' + $(this).parents('.drv-upload-input').attr('data-id');
        url += (url.indexOf('?') === -1 ? '?' : '&') + 'sendto=drvImceInputSlider.urlSendto&inputId=' + inputID + '&type=link';
        $('#drv-upload-' + inputID).focus();
    ...
    //Select IMCE file, display image demo and set val for upload input.
      ...
      $('.imce-url-input').not('.imce-url-processed').addClass('imce-url-processed').each(function(){
        ...
        el.before('<a href="#" class="imce-url-button"><span>Open File Browser</span></a> | <a href="#" class="dardev-field-upload-remove">Remove</a>');
      });

      var drvImceInputSlider = window.drvImceInputSlider = window.drvImceInputSlider || {
         urlSendto: function(File, win) {
          ...
          win.close();
          if (el) {
            var base_path = drupalSettings.dardev_sliderlayer.base_path;
            var url_new = '/' + url.replace(base_path, '');
            console.log(url);

            $(el).val(url_new);
            $(el).parents('.drv-upload-input').find('.dardev-image-demo').attr('src', url);
            $('#dardev_slider_single').css('background-image', 'url(\'' + url + '\')');
          }
        }
      };

      $(document).ready(function(){
        $('.dardev-field-upload-remove').on('click', function(e){
          e.preventDefault();
          $(this).parents('.drv-upload-input').find('.dardev-image-demo').attr('src', '');
          $(this).parents('.drv-upload-input').find('.imce-url-input').val('');
          $('#dardev_slider_single').css({'background-image': 'url("")'});
        });
      })
