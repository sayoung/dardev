...
class ImportForm implements FormInterface {
   ...
   */
   public function buildForm(array $form, FormStateInterface $form_state) {
      ...
      if (is_numeric($bid)) {
        $bblock = \Drupal::database()->select('{dardev_pagebuilder}', 'd')
           ->fields('d')
           ->condition('id', $bid, '=')
           ->execute()
           ->fetchAssoc();
      ...
      if($bblock['id']==0){
        \Drupal::messenger()->addMessage('Not found dardev block builder !');
        return false;
      }
      $form = array();
      $form['id'] = array(
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($form['id']['#value']) {
      ...
      $response = new \Symfony\Component\HttpFoundation\RedirectResponse(Url::fromRoute('dardev_pagebuilder.admin.edit', array('bid'=>$id))->toString());
      $response->send();
    }  
  }
}