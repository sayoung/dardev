...
class element_drv_row{
	...
	public function render_content( $settings = array(), $content = '' ) {
         ...
         $class ? $array_class[]	= $class : false;
			...
			if( $bg_image){
				...
				if($bg_attachment=='fixed'){
					$array_class[] = 'drv-parallax-background';
				}
				if($bg_attachment=='bg-fixed'){
					$array_class[] = 'drv-fixed-background';
				}
			}
		...
		<section class="section <?php print trim($row_class) ?>" <?php if($row_id) print 'id="'.$row_id.'"' ?>  style="<?php print $row_style ?>" <?php if($data_bg_video) print $data_bg_video; ?> <?php if($row_name) print 'data-onepage-title="'.$row_name.'"'; ?>>
		  	<?php if($icon){ ?><span class="icon-row <?php print $icon ?>"></span><?php } ?>
		    	<div class="bb-inner <?php if($style_space) print $style_space; ?>">  
			  	...
			  	?>
			    <div style="background-repeat: <?php print $bg_repeat; ?>;background-position:<?php print $bg_position ?>;<?php print ($bg_size=='width-100'?'background-size: 100%;':'')?>" class="drv-parallax-inner skrollable skrollable-between <?php print $row_bg_size ?>" <?php print $data_parallax ?>></div>
			  <?php } ?>
			</section>  
