<?php
function dardev_sliderlayer_load($sid) {
  $result = \Drupal::database()->select('{dardev_sliderlayers}', 'd')
      ->fields('d')
      ->condition('id', $sid, '=')
...
function dardev_sliders_by_group($gid=0) {
  $result = \Drupal::database()->select('{dardev_sliderlayers}', 'd')
          ->fields('d')
...
function dardev_slider_load_frontend($sid=0) {
  $group = \Drupal::database()->select('{dardev_sliderlayergroups}', 'd')
          ->fields('d')
          ->condition('id', $sid, '=')
          ->execute()
          ->fetchObject();

  $slides = \Drupal::database()->select('{dardev_sliderlayers}', 'd')
            ->fields('d')
...
function getListSliderGroups(){
    $result = \Drupal::database()->select('{dardev_sliderlayergroups}', 'd')
          ->fields('d')
          ->execute()
          ->fetchObject();
    return $result;
...
function dardev_sliderlayer_export($gid){
  $result = new stdClass();
    ...
    $sliders = dardev_sliders_by_group($gid);
