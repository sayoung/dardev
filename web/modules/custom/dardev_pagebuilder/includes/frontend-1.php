...
function dardev_pagebuilder_frontend( $params ) {
	...
	if ( ! empty( $content ) ) {
		foreach ( $content as $k => $row ) {
			print (dardev_pagebuilder_render_el( $row ) );
		}
	}
 ...
 function dardev_pagebuilder_render_el( $data = array(), $content = '' ) {
		...
		if ( ! empty( $data['columns'] ) || ! empty( $data['elements'] ) ) {
			...
			foreach ( $has_els as $element ) {
				...
				if ( $subs ) {
					print dardev_pagebuilder_render_el( $element, $subs );
				}
			}
			$content = ob_get_clean();
		}

		if ( isset( $data['element_name'] ) && $data['element_name'] == 'drv_column' && ! empty( $data['col_lg'] ) ) {
			$settings['col_lg'] = $data['col_lg']; 
		}

		if ( isset( $data['element_name'] ) ) {
			if ( ! in_array( $data['element_name'], array( 'drv_row', 'drv_column' ) ) ) {
				ob_start();
				print dardev_pagebuilder_render_element($data['element_name'], $settings, $content);
				$content = ob_get_clean();
				return $content;
			}
			return dardev_pagebuilder_render_element($data['element_name'], $settings, $content);
		}
	}
