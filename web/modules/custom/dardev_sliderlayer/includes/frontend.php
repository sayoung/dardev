<?php
function dardev_sliderlayer_block_content($sid) {
  global $base_url;
  $slideshow = dardev_slider_load_frontend($sid);
  if(!$slideshow) return 'No slider selected';
  $module_path = \Drupal::service('extension.list.module')->getPath('dardev_sliderlayer');
  ...
  return dardev_sliderlayer_slides($slide_settings);

}

function dardev_sliderlayer_slides($vars){
  global $base_url;
  $slides = $vars['slides'];
  $scount = $vars['scount'];
  $settings = $vars['settings'];
  $vars['attributes_array']['class'] = 'dardev_sliderlayer rev_slider_wrapper fullwidthbanner-container';
  $style = array();
  ...
  foreach($slides as $slide){
    if($slide->status){
      ...
      $vars['content'] .= dardev_sliderlayer_slide($slide_settings, $i);
      $i = $i + 1;
    }  
  }
...
function dardev_sliderlayer_slide($vars, $index){
  global $base_url;
  $module_path = \Drupal::service('extension.list.module')->getPath('dardev_sliderlayer');
  $slide = $vars['slide'];
  $layers = $slide->layers;
  ...
  if(isset($slide->overlay_enable) && $slide->overlay_enable=='on'){
    $vars['attributes_array']['class'] = 'dardev-overlay ';
  }
  ...
  foreach($layers as $layer){
    ...
    $vars['content'] .= dardev_sliderlayer_layer($layer_settings, $layer_count, $slide_id);
    $layer_count ++;
  }
...
function dardev_sliderlayer_layer($vars, $layer_count, $slider_id){
  global $base_url;
  $module_path = \Drupal::service('extension.list.module')->getPath('dardev_sliderlayer');
  $layer = $vars['layer'];
  $scount = $vars['scount'];
 ...
 // $vars['attributes_array']['data-transform_idle'] ="o:1;";
  ...
  if($layer->incomingclasses){
    $tmp = dardevGetArrAnimations()[$layer->incomingclasses];
    if($tmp){
  ...
  if($layer->outgoingclasses){
    $tmp_out = dardevGetArrEndAnimations()[$layer->outgoingclasses];
    if($tmp_out){
